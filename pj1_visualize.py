...
class RobotVisualization:
    def __init__(self, num_robots, width, height, 
        ...
        self.master = Tk(className="myRobot!")
    ...
    def _draw_robot(self, position, direction):
        ...
        x2, y2 = self._map_coords(x + 0.8 * math.sin(math.radians(d1)),
                                  y + 0.8 * math.cos(math.radians(d1)))
        x3, y3 = self._map_coords(x + 0.8 * math.sin(math.radians(d2)),
                                  y + 0.8 * math.cos(math.radians(d2)))
...
example = RobotVisualization(100,600,600,0,0.2)
example.done()
#num_robots, width, height, furniture_tiles, delay = 0.2
